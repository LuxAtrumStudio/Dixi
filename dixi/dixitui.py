...
def action(key):
    ...
    if 'options' in PANNELS[pannel] and PANNELS[pannel]['selection'] != -1:
        ...
        if option == 'Login':
            dixi.user.login_card(dixi.config.get('color'))
        elif option == 'Register':
            dixi.user.register_card(dixi.config.get('color'))
        elif option == 'Logout':
            dixi.user.logout_card(dixi.config.get('color'))
        elif option == 'Delete':
            dixi.user.delete_card(dixi.config.get('color'))
        elif option == 'Config':
            dixi.config.addr(dixi.config.get('color'))
            global current_channel
            current_channel = None
        elif option == 'Create Channel':
            dixi.channel.create_card(dixi.config.get('color'))
        elif option == 'Delete Channel':
            dixi.channel.delete_card(dixi.config.get('color'))
        elif option == 'Quit':
            close()
        load()
...
def display_channel():
    ...
    if channel in POSTS:
        for post in POSTS[channel]:
            print_message(post, longest, dixi.config.get('color'), PANNELS['main']['pannel'])
